...
void agregarAlumno(){
    ...
    if(pos >= 0){
        cout<<"YA HAY UN REGISTRO CON ESE LEGAJO EN EL ARCHIVO"<<endl;
        return;
    }
    obj.Cargar(legajo);
    pos = arc.buscarRegistro(obj.getLegajo());
    if(pos >= 0){
        cout<<"YA HAY UN REGISTRO CON ESE LEGAJO EN EL ARCHIVO"<<endl;
        return;
    }
...
//    AlumnoVieja obj;
//    Alumno aux;
//    ArchivoAlumnos arc("Alumnos.aux");
...
//        aux.setFechaInscripcion(obj.getFechaInscripcion());
//        aux.setEstado(true);
//        aux.Mostrar();
//        arc.grabarRegistro(aux);
//    }
}
...
void modificarMail(){
    ...
    int pos = arc.buscarRegistro(legajo);
    if(pos < 0){
        cout<<"NO HAY UN ALUMNO CON ESE LEGAJO EN EL ARCHIVO"<<endl;
        return;
    }
    Alumno obj = arc.leerRegistro(pos);
...
/**
VAMOS A HABLAR DE ELIMINACION DE REGISTROS:
LA ELIMINACION DE UN REGISTRO PUEDE HACRESE DE DOS FORMAS DISTINTAS:
-ELIMINACION FISICA: CONSISTE EN EL BORRADO DE LA INFORMACION DEL ARCHIVO
-ELIMINACION LOGICA: CONSISTE EN CONSERVAR LA INFORMACION PERO SE MARCA
EL REGISTRO COMO ELIMINADO. ESTO SE HACE MEDIANTE EL USO DE UNA NUEVA
PROPIEDAD DESTINADA A TAL FIN. EL SISTEMA DEBE IGNORAR A LOS REGISTROS
MARCADOS COMO ELIMINADOS (O DADOS DE BAJA).
*/
...
void bajaLogicaAlumno(){
    ...
    if(obj.getEstado() == false){
        cout<<"EL ALUMNO YA SE ENCUENTRA DADO DE BAJA"<<endl;
        return;
    }
    obj.setEstado(false);
    arc.modificarRegistro(obj, pos);
...
int main(){
    ...
    while(true){
        ...
        cout<<"6-MODIFICAR MAIL ALUMNO"<<endl;
        cout<<"7-DAR DE BAJA LOGICA A UN ALUMNO"<<endl;
        cout<<"0-SALIR"<<endl;
        cout<<"===================="<<endl;
